...
UI library (`D:\evo`). This task creates a new, standalone hook with no consumers yet —
later tasks will import it. Zero runtime dependencies allowed (project policy).
...
- `interface UseAnchoredPositionOptions<T>` — `{ open, anchorRef: RefObject<T|null>, placement?, offset?, viewportPadding?, matchAnchorWidth? }`
- `interface AnchoredPosition` — `{ floatingRef: RefObject<HTMLDivElement|null>, floatingStyles: CSSProperties, arrowStyles: CSSProperties, placement: AnchorSide, ready: boolean }`
- Do NOT export from `src/index.ts` — this is an internal utility.

## Step 1: Create `evo-ui/src/hooks/useAnchoredPosition.ts` with EXACTLY this content
...
Expected: build succeeds with no TypeScript errors. Capture the tail of the output as evidence.

## Step 3: Commit (stage ONLY the new hook file)
```
...
```
Do NOT run `git add -A`. Do NOT commit `dist/` (git-ignored) or anything under `.superpowers/`.

## Report contract
Write your full report to `D:\evo\.superpowers\sdd\vap-task-1-report.md` (what you did, the
build result tail, the commit hash, and any concerns). Return ONLY: status
